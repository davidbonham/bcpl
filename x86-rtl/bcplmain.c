...
{
    extern int ioinit(bcplword_t stdin, bcplword_t stdout, bcplword_t stderr) __asm__("blib.ioinit");
    extern int ioterm() __asm__("blib.ioterm");
    extern int blib_rditem_add(char* c_string) __asm__("blib.rditem_add");
    extern int blib_rditem_end() __asm__("blib.rditem_end");
    ioinit(0, 1, 2);
    ...
    if (argc > 1)
    {
        ...
        {
            if (i > 1) blib_rditem_add(" ");         // Treat args fred and jane as 'fred jane' not 'fredjane'
            blib_rditem_add(argv[i]);
        }
        blib_rditem_end();
    }
    ...
    __bcpl_global_vector[G_FILESYS]         = (bcplword_t)environ;
...
//        putchar('\n');
//        if (__bcpl_global_vector[G_COS] != 0)
//        {
//            bcplword_t (*endwrite_fn)() = (bcplword_t (*)(bcplword_t))__bcpl_global_vector[G_ENDWRITE];
//            endwrite_fn();
//        }
        ioterm();
        return result;
