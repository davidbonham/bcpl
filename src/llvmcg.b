...
LET LLVM_init() = sys(Sys_ext, EXT_LLVM_init)
LET LLVM_context_create() = sys(Sys_ext, EXT_LLVM_context_create)
LET LLVM_module_create_with_name_in_context(name, context_ref) = sys(Sys_ext, EXT_LLVM_module_create_with_name_in_context, name, context_ref)
LET LLVM_print_module_to_file(module_ref, output_path, out_error) = sys(Sys_ext, EXT_LLVM_print_module_to_file, module_ref, output_path, out_error)
LET LLVM_create_builder_in_context(context_ref) = sys(Sys_ext, EXT_LLVM_create_builder_in_context, context_ref)
LET LLVM_dispose_builder(builder_ref) BE sys(Sys_ext, EXT_LLVM_dispose_builder, builder_ref)
LET LLVM_add_global(module_ref, type_ref, name) = sys(Sys_ext, EXT_LLVM_add_global, module_ref, type_ref, name)
LET LLVM_int64_type_in_context(context_ref) = sys(Sys_ext, EXT_LLVM_int64_type_in_context, context_ref)
LET LLVM_double_type_in_context(context_ref) = sys(Sys_ext, EXT_LLVM_double_type_in_context, context_ref)
...
{ LET treesize = 0
  ...
  writef("*nBCPL (LLVM) %n bit with the FLT feature*n", bitsperword)
...
{ 
    // Allocate a block of three cells from the falling top of our heap
    // memory
    LET p = dvect - 3

    // If it's reached the rising dvece, we've run out od space
  IF dvece>p DO { errmax := 0        // Make it fatal.
                  trnerr("More workspace needed")
                }

  // Otherwise, initialise the words and then allocate
  p!0, p!1, p!2 := x, y, z
  dvect := p
...
AND translate(x) BE
{ 
    // treevec points to the start of our block of heap and treep points
    // past its end. 
...
//   writef("Undeclare %s*n", name+2)
  ...
  // Translate the rest of the section, if there is one
  trans(x, 0)

  // Now the section has been translated, we can emit the global vector
  ...
  // At this stage, trans() will have dealt with any semantic errors and
  // failed or tweaked the ae tree so we can assume all is well when we 
  // emit our ir
  emit_ir(x)
}
...
AND emit_ir(x) BE
$(
    // When we output our LLVM IR, we need a location to receive the text
    // of any error message it may return. We are responsible for returning
    // this text to the BCPL heap.
    LET error = ?

    // Start the LLVM IR output
    LLVM_init()
    llvm_context_ref := LLVM_context_create()
    llvm_current_module := 0
    llvm_next_global := 0

    // All SECTION and NEED statements must precede the other block-head
    // declarations (such as globals and procedures)
    WHILE x ~= 0 & (h1!x = s_section | h1!x = s_needs) DO
    $(
        // Unpack the NEEDS or SECTION node and emit it
        LET op, argument = h1!x, h2!x        
        SWITCHON op INTO
        $(
            // Note that in a string AE node, the first field is the node type as
            // usual but the rest of the node is the string itself, hence the @.
            CASE s_section:  emit_section(@h2!argument); ENDCASE
            CASE s_needs:    emit_needs(@h2!argument);   ENDCASE
        $)

        // Advance to the next node in the AE tree if there is one
        x := h3!x
    $)

    // If we didn't see a section, default to BCPLMAIN
    IF llvm_current_module = 0 THEN emit_section("BCPLMAIN")

    // Emit IR code for all the nodes in this command. 0 means there is no
    // trailing return or jump required.
    emit_command(x, 0)

    // Finally, write out the LLVM IR of the module we have translated and 
    // handle any error.
    LLVM_print_module_to_file(llvm_current_module, "output.ir", @error)
    IF error ~= 0 THEN
    $(
        writef("Error on writing LLVM IR: %s*n", error)
        freevec(error)
    $)

$)

AND emit_command(c, following_action) BE
$(
    // Because this may be a sequence (s_seq) node which we will traverse
    // at this level advancing c, keep going until we reach a terminating 
    // node. Non-sequence nodes will return out of their cases.
    UNTIL c = 0 DO
    $(
        LET op = h1!c
        writef("%D: %S*N", c, opname(h1!c))
        SWITCHON op INTO
        $(
            CASE s_seq:
            $(
                LET child_command, next_command = h2!c, h3!c
                emit_command(child_command, 0)
                c := next_command
            $)
            ENDCASE
            ...
            CASE s_global: CASE s_static: CASE s_manifest:
                emit_cdefs(c, following_action)
                RETURN

            DEFAULT: 
                writef("Unknown AE type at %D: %S*N", c, opname(op))
                RETURN
                // Currently, we ignore unimplemented nodes
                //trnerr("Compiler error in emit_command, op = %s", opname(op))          
        $)
    $)

    // Having completed the sequence, emit any terminating code
    emit_following(following_action)
$)

AND emit_cdefs(c, following_action) BE
$(
    // We are given a list of global, static or manifest constant declarations.
    // h1 is the type, h2 is the list of definitions and h3 is the command (a
    // sequence, probably) to translate in the new context created by declaring
    // these constants
    LET type, cdefs, nested_command = h1!c, h2!c, h3!c

    // Note the state of name declarations on entry to this routine scope
    // so we can restore it at the end
    LET start_of_scope = dvece

    // When a name has no specified value, the value follows sequentially
    // after the previous one provided it is of the same type, otherwise
    // we start again at 0. Keep track of the previous value and its type
    LET value, previous_type = 0, 0

    UNTIL cdefs = 0 DO
    $(
        LET rest, name, expression = h2!cdefs, h3!cdefs, h4!cdefs

        // Deal with floating point values which are inidcated by a name
        // field that is (S_FLT, NAME). If we have one, transform the operation
        // into its floating equivalent and extract the real name
        LET ftype, float_flag = type, FALSE
        IF h1!name = s_flt DO 
        $(
            // The value associated with the type is floating poing only for 
            // STATIC and MANIFEST - for GLOBAL it is still the global number
            name, ftype := h2!name, type | s_fltbit
             IF type ~= s_global DO float_flag := TRUE
        $)

        // If we have an expression, it must be a constant which we evaluate
        // as a float or a word accordingly. IF not, we deduce it from that
        // last value
        TEST expression ~= 0 THEN
            value := evalconst(expression, float_flag)
        ELSE
        $(
            TEST previous_type = 0 THEN
                // No previous type - start at zero
                value := float_flag -> flt0, 0
            ELSE SWITCHON ftype INTO
            $(
                // STATIC declarations are always zero
                CASE s_static:  value := 0     ENDCASE
                CASE s_fstatic: value := flt0  ENDCASE

                // For MANIFEST the value is one larger than the previous
                // value, converted to type, if necessary.
                CASE s_manifest:
                    IF previous_type = s_flt DO value := sys(Sys_flt, fl_fix, value)
                    value +:= 1
                ENDCASE           

                CASE s_fmanifest:
                    IF previous_type = s_notflt DO value := sys(Sys_flt, fl_float, value)
                    value := sys(Sys_flt, fl_add, value, flt1)
                ENDCASE

                // For GLOBAL the value is a global number one larger than 
                // the previous one.
              CASE s_global: CASE s_fglobal: value +:= 1 ENDCASE
            $)            
        $)

        // We now have the new value. Record its type for next time
        previous_type := float_flag -> s_flt, s_notflt

        // We need to generate the location for the static variable and
        // initialise it. It will need a unuque label.
        TEST type = s_static THEN
        $(
            LET variable_type = float_flag -> LLVM_double_type_in_context(llvm_context_ref), LLVM_int64_type_in_context(llvm_context_ref)
            LET constant = value
            value := nextlab()

            //this needs to be dso_local not external and also given its initial value
            LLVM_add_global(llvm_current_module, variable_type, @h3!name)        
            LLVM_add_global(llvm_current_module, variable_type, @h3!name)        
        $)
        ELSE IF type = s_global UNLESS 0 <= value <= 1023 DO
        $(
            // Global is not sane
            trnerr("Global number %n too large for: %s*n", value, @h3!name)
        $)

        // For the other types, we already have the value for the symbol.
        // In all cases, declare it
        addname(name, ftype, value)

        // Advance to the next name being declared, if there is one
        cdefs := h2!cdefs
    $)

    // Now we have declared all the symbols in this command, we have a new 
    // new context in which to evaluate the child nodes. Set up its scope
    // as usual and then emit its code
    decllabels(nested_command)
    emit_command(nested_command, following_action)

    // Discard the context we have created now it is going out of scope
    undeclare(start_of_scope)
$)

AND emit_let(c) BE
$(
    // There are multiple types of let
    LET let_type = h2!c
    SWITCHON let_type INTO
    $(
        CASE s_rtdef:
            emit_rtdef(c)
        ENDCASE
...
/*
      ...
      undeclare(e)
*/    
    // Unpack the RTDEF node
    LET name, arguments, command = h2!c, h3!c, h4!c

    // Note the state of name declarations on entry to this routine scope
    // so we can restore it at the end
    LET start_of_scope = dvece

    // Declare the name. After this the name will know if it is a global 
    // or not and what its label is as h2 will point to its entry in the
    // declaration vector
    h5!c := nextlab()
    declstat(h2!c, h5!c)

    // Check that having added this definition, we don't have any duplicate
    // declarations
    checkdistinct(start_of_scope)
...
AND emit_following(action) BE
$(
    // Nothing yet
$)

AND emit_section(module_name) BE
$(
      writehex(llvm_current_module, 16)
      newline()
      writehex(llvm_context_ref, 16)
      newline()

    llvm_current_module := LLVM_module_create_with_name_in_context(module_name, llvm_context_ref)
    writef("module name %s*n", module_name)
$)

AND emit_needs(symbol) BE
$(
    writef("needs external reference %s*n", symbol)
$)
  ...
  { DEFAULT: trnerr("Compiler error in Trans, op = %s",
    ...
    { LET cc = casecount
      ...
      { LET name = h3!cdefs
        ...
        context, comline := cdefs, h5!cdefs
...
/**
 * Add the declaration for this node to end of the declared items and update
 * the name cell to reference this declaration.
 *
 * @param x     the name to be declared
 * @param lab   the label allocated to the item 
 */ 
AND declstat(x, lab) BE
{ 
    // Get the name cell for the name x (which may or may not already exist)
    LET c = cellwithname(x)

    // Get the flags for this name, possibly including the floating version
    // so remove the later
    LET fk = h2!c
    LET k = fk & s_fltmask 

    TEST k=s_global THEN 
    { 
        // This name is in the global vector. Add a new three-word node to our
        // list of definitions of globals - we know the number and we have been
        // given the label
        LET gn = h3!c
        gdeflist := newblk(gdeflist, gn, lab)
        gdefcount := gdefcount + 1

        // Add the name to the end of the declaration vector and record
        // the position of it in x (at h2)
        addname(x, s_global, gn)

        IF xrefing DO xref(x, (fk=s_fglobal -> "FG:", "G:"), gn, h1!context)
        IF gdefsing DO writef("G%i3 = %s*n", gn, @h3!x)
    }
    ELSE 
    {
        // This is a local, static, manifest or label. Add it as a label
        // to the end of the declararations and update x to remember where
        // it is.
        addname(x, s_label, lab)
        IF xrefing DO xref(x, (fk=s_flocal -> "FF:", "F:"), lab, h1!context)
    }
}

/**
 * Declare all of the labels for items that require them in the tree given.
 * (These items are globals, manifests, statics and case labels). The name
 * nodes are updated to reference their declarations in the declaration 
 * vectors.
 *
...
{ 
    // Add the name to end of the declaration vector with the given flags
    // and label.
    LET p = dvece + 3
    IF p>dvect DO trnerr("More workspace needed")
    h1!dvece, h2!dvece, h3!dvece := name, k, a

    // Record the location of the declaration in the name node too
...
AND undeclare(e) BE 
{ 
    // For all names in the declaration vecor at e onwards to the end of the
    // declaration vector (that is, all names declared at e or later), remove
    // them from the vector and mark all of the associated names undeclared.
...
{ 
    // n is a name node not prefixed by FLT. Look at the name cell to get
    // its entry in the declaration vector.
    LET t = h2!n

    IF t RESULTIS t  // It has been looked up before, as it points to its entry

    // This hame hasn't been looked up. It may have a declaration though so
    // search from the end of the vector (the most recent scope) and look
    // for the name.
    t := dvece
...
/**
 * Traverse the part of the ae tree supplied and allocate labels for the
 * nodes within it that need them. (globals, manifests, statics, case labels)
 * 
 * For each allocated label, declare it appropriately in the declarations 
 * vector.
 */
